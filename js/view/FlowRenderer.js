...
export class FlowRenderer {
    ...
    render(model) {
        ...
        model.nodes.forEach(n => {
            ...
            this.drawNode(n, nodeStats, isSelected);
        });
    }
    ...
    drawNode(node, nodeStats, isSelected = false) {
        const x = node.x || 0;
        const y = node.y || 0;
        ...
        this.ctx.fill();
        this.ctx.strokeStyle = isSelected ? 'var(--accent-primary)' : '#1e293b'; 
        this.ctx.lineWidth = isSelected ? 5 : 3;
        this.ctx.stroke();

        // Highlighting Ring for selection
        if (isSelected) {
            this.ctx.beginPath();
    ...
    drawLink(s, t, amount) {
        ...
        if (amount > 0) {
            ...
            const text = this.formatCurrency(amount);
            this.ctx.font = '11px Outfit';
            const tm = this.ctx.measureText(text);
            
            this.ctx.fillStyle = 'rgba(15, 23, 42, 0.8)';
            this.ctx.roundRect(midX - tm.width/2 - 5, midY - 10, tm.width + 10, 20, 4);
            this.ctx.fill();
    ...
    findNodeAt(nodes, x, y) {
        return nodes.find(n => {
            const dx = x - (n.x || 0);
            const dy = y - (n.y || 0);
            return (dx * dx + dy * dy) <= (28 * 28); // 20 radius -> 28 hit area
        });
    }

    findLinkAt(links, nodes, x, y) {
        return links.find(l => {
