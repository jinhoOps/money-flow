...
export class UIManager {
    ...
    setupEventListeners() {
        ...
        // ... (Canvas DragDrop listeners from previous step, assumed retained) ...
        ...
        canvas.addEventListener('mousedown', (e) => { /*...*/ 
            ...
            if (node) { isDragging = true; draggedNode = node; canvas.style.cursor = 'grabbing'; }
        });
        canvas.addEventListener('mousemove', (e) => { /*...*/
             if (isDragging && draggedNode) {
                ...
                this.renderer.render(this.model);
                 ...
                 const rect = canvas.getBoundingClientRect(); // re-calc for simplicity
                 const x = e.clientX - rect.left;
                 const y = e.clientY - rect.top;
                 const node = this.renderer.findNodeAt(this.model.nodes, x, y);
        ...
        canvas.addEventListener('mouseup', () => { /*...*/
            if (isDragging) { isDragging = false; draggedNode = null; canvas.style.cursor = 'default'; this.storage.save(this.model); }
        });
        // ...
    ...
    handleNodeSubmit() {
        ...
        const value = Number(document.getElementById('nodeValue').value);
        ...
        if (id) {
            this.model.updateNode(Number(id), { type, name, value, interestRate, owner });
        } else {
            const n = this.model.addNode(type, name, value, 0, 0, owner);
            this.model.updateNode(n.id, { interestRate }); // Set interest
        }
        this.saveAndRender();
    ...
    renderEditor() {
        ...
        this.els.nodeList.innerHTML = filteredNodes.map(n => `
            <li class="editor-item">
                <div class="item-info">
                    <span class="item-name" style="color: var(--accent-primary)">${n.name}</span>
                    <span class="item-type">${n.type}</span>
                    <span class="item-owner" style="font-size: 0.7rem; color: var(--text-secondary)">(${n.owner || '나'})</span>
                </div>
